...
package com.zhy.dialtong.sipua.sipua;
...
import com.zhy.dialtong.sipua.net.KeepAliveSip;
import com.zhy.dialtong.sipua.ui.ChangeAccount;
import com.zhy.dialtong.sipua.ui.LoopAlarm;
...
public class SipdroidEngine implements RegisterAgentListener {
	...
	public boolean StartEngine() {
			...
			if (wl == null) {
				if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).contains(com.zhy.dialtong.sipua.ui.Settings.PREF_KEEPON)) {
					Editor edit = PreferenceManager.getDefaultSharedPreferences(getUIContext()).edit();
	
					edit.putBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_KEEPON, true);
					edit.commit();
				}
			...
			for (UserAgentProfile user_profile : user_profiles) {
				if (wl[i] == null) {
					...
					if (!PreferenceManager.getDefaultSharedPreferences(getUIContext()).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_KEEPON, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_KEEPON)) {
		...
		//			SipStack.log_path = "/data/data/org.sipdroid.sipua";
					...
					String version = "Sipdroid/" + DialFragment.getVersion() + "/" + Build.MODEL;
			...
			register();
			listen();//Receives incoming calls (auto accept) 
	...
	public void registerMore() {//���ж��˻�״̬��ע��SIP��������ʾ��֪ͨ���ϵ�ͼ��
		IpAddress.setLocalIpAddress();
		int i = 0;
		for (RegisterAgent ra : ras) {
			try {
