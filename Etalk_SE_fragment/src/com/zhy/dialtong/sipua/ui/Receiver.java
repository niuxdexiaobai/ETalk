...
import com.zhy.dialtong.sipua.media.RtpStreamSender;
import com.zhy.dialtong.sipua.phone.Call;
import com.zhy.dialtong.sipua.phone.Connection;
import com.zhy.dialtong.sipua.sipua.SipdroidEngine;
import com.zhy.dialtong.sipua.sipua.UserAgent;
//Receiver在显示进度以及定位更新等信息，添加了关于、退出、设置等菜单调用
	public class Receiver extends BroadcastReceiver {//接收器
...
//					Bluetooth.init();
		...
		public static void stopRingtone() {
			if (v != null)
				v.cancel();//Turn the vibrator off. 
			if (Receiver.oRingtone != null) {
				Ringtone ringtone = Receiver.oRingtone;
				oRingtone = null;
				ringtone.stop();//Stops a playing ringtone.
			}
		}
		...
		public static void onState(int state,String caller) {
			...
			if (call_state != state) {
				...
				switch(call_state)
				{
				case UserAgent.UA_STATE_INCOMING_CALL://1
					enable_wifi(true);
					...
					if ((pstn_state == null || pstn_state.equals("IDLE")) &&
							PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_AUTO_ON, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_AUTO_ON) &&
						...
						if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {				 
							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_SIPRINGTONE,
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			...
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_REGISTRATION, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_REGISTRATION))
				text = null;
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
			        	...
			        	if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_NOTIFY, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_NOTIFY)) {
		        	...
		        	default:
		        		if (type >= REGISTER_NOTIFICATION && mSipdroidEngine != null && type != REGISTER_NOTIFICATION+mSipdroidEngine.pref &&
		        			...
		        			notification.contentIntent = PendingIntent.getActivity(mContext, 0,
		        					createIntent(DialFragment.class), 0);
					...
					if (base != 0) {
						...
						if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_POS, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POS))
		...
		static void updateAutoAnswer() {
			if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_AUTO_ONDEMAND, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_AUTO_ONDEMAND) &&
				DialFragment.on(mContext)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_AUTO_DEMAND, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_AUTO_DEMAND))
					updateAutoAnswer(1);
				else
					updateAutoAnswer(0);
			} else
		...
		public static void pos(boolean enable) {
			...
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_POS, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POS) ||
			PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_POSURL, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POSURL).length() < 1) {
				if (lm != null && am != null) {
					pos_gps(false);
					pos_net(false);
					lm = null;
			...
			pos_gps(false);
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && DialFragment.on(mContext) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_POS, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POS) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_POSURL, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POSURL).length()>0) {
		...
		static void enable_wifi(boolean enable) {
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_OWNWIFI, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_OWNWIFI))
				return;
			if (enable && !PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_WIFI_DISABLED, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_WIFI_DISABLED))
        		return;
    		...
    		edit.putBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_WIFI_DISABLED,!enable);
    		edit.commit();
    		/*
    		if (enable) {
    		...
    		*/
    		wm.setWifiEnabled(enable);//Enable or disable Wi-Fi.
		}
		...
		public static void url(final String opt) {
	        (new Thread() {
				public void run() {
					try {
				        URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_POSURL, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_POSURL)+
				        		"?"+opt);
				        BufferedReader in;
						in = new BufferedReader(new InputStreamReader(url.openStream()));
				        in.close();
					} catch (IOException e) {
						if (!DialFragment.release) e.printStackTrace();
					}
		...
		public static void alarm(int renew_time,Class <?>cls) {
       		if (!DialFragment.release) Log.i("SipUA:","alarm "+renew_time);
		...
		public static void progress() {//通话时候的状态
		...
		static boolean on_vpn() {
			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_ON_VPN, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_ON_VPN);
		}
		
		static void on_vpn(boolean enable) {
    		Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();
    		
    		edit.putBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_ON_VPN,enable);
    		edit.commit();
		}
		
		public static boolean isFast(int i) {//��ȡWiFi״̬����GSM
        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
        	WifiInfo wi = wm.getConnectionInfo();

        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_USERNAME+(i!=0?i:""),"").equals("") ||
        			PreferenceManager.getDefaultSharedPreferences(mContext).getString(com.zhy.dialtong.sipua.ui.Settings.PREF_SERVER+(i!=0?i:""),"").equals(""))
        		return false;
        	if (wi != null) {
        		if (!DialFragment.release) Log.i("SipUA:","isFastWifi() "+WifiInfo.getDetailedStateOf(wi.getSupplicantState())
        				+" "+wi.getIpAddress());
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		on_wlan = true;
	        		if (!on_vpn())
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_WLAN+(i!=0?i:""), com.zhy.dialtong.sipua.ui.Settings.DEFAULT_WLAN);
	        		else
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_VPN+(i!=0?i:""), com.zhy.dialtong.sipua.ui.Settings.DEFAULT_VPN);  
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_3G+(i!=0?i:""), com.zhy.dialtong.sipua.ui.Settings.DEFAULT_3G);
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
       			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_EDGE+(i!=0?i:""), com.zhy.dialtong.sipua.ui.Settings.DEFAULT_EDGE);
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (!DialFragment.on(context)) return;
        	if (!DialFragment.release) Log.i("SipUA:",intentAction);
		    ...
		    if (intentAction.equals(WifiManager.WIFI_STATE_CHANGED_ACTION)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_SELECTWIFI, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_SELECTWIFI))
	        ...
	        if (intentAction.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
	        	if (SystemClock.uptimeMillis() > lastscan + 45000 &&
	        			PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(com.zhy.dialtong.sipua.ui.Settings.PREF_SELECTWIFI, com.zhy.dialtong.sipua.ui.Settings.DEFAULT_SELECTWIFI)) {
		        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
		        	WifiInfo wi = wm.getConnectionInfo();
		        	...
		        	if (configurations != null) {
		                ...
		                if (bestconfig != null &&
		               		...
		               		if (!DialFragment.release) Log.i("SipUA:","changing to "+bestconfig.SSID);
